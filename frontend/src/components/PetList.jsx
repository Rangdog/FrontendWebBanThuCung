...
const PetList = () => {
    ...
    return (
      <>
        <Grid container spacing={1} className='gid'> {/* Grid container with spacing between items */}
          {currentPets.map((pet, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}> {/* Define grid layout */}
              <PetCard pet={pet} hinhAnh = {petTest[index].hinhAnh} />
            </Grid>
          ))}
        </Grid>
          <Pagination className='pagination-container' count={totalPages}  page={currentPage}  onChange={handleChange} variant="outlined" shape="rounded" />
      </>
    );
};
